...
resource "time_sleep" "wait_90_seconds" {
  depends_on      = [azurerm_linux_virtual_machine.linuxvmalias]
  create_duration = "90s"
}
...
resource "null_resource" "sync_app1_static" {
  ...
  connection {
    type        = "ssh"
    host        = azurerm_linux_virtual_machine.linuxvmalias.public_ip_address
    user        = azurerm_linux_virtual_machine.linuxvmalias.admin_username
